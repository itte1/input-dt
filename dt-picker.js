...
export class DtPicker extends HTMLElement {
  constructor() {
    super()
  ...
  _dispatch() {
    ...
    this.querySelectorAll('[dt-picker-input]').forEach(el => {
      switch (el.type) {
        case 'text':
    ...
    this.querySelectorAll('[dt-picker-display]').forEach(el => {
      replaceChildren(el, [t(this.format(this.value))])
    })
  }

  _attach() {
    this.querySelectorAll('input[dt-picker-input]:not([readonly])').forEach(el => {
      el.addEventListener('focus', this.open)
    })
    this.querySelectorAll('[dt-picker-open]').forEach(el => {
...
globalThis.customElements.define('dt-picker', DtPicker)